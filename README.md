...
一个可被本地智能体使用的 HandaaS 企业大数据 Skill。用户只需要说"使用企业大数据，分析华为技术有限公司的工商信息、股东与对外投资"，智能体会自动连接 MCP 服务、获取原始数据并编排为结构化报告（HTML + Markdown + JSON + PDF 四件套），内嵌 ECharts 5.x 可视化图表，离线可打开。

> Skill 本身不托管用户数据或凭证。推荐通过 HandaaS 平台创建 `enterprise-mcp-server` 在线 MCP 对接器并使用 token；也支持连接本地部署的 `enterprise-mcp-server`。凭证由 MCP Server 自己的 `.env` 持有，不在 Skill 内处理。
...
```text
请帮我安装并调试 HandaaS enterprise-skill：先完整读取 https://github.com/handaas/enterprise-skill 这个项目的 README.md，再克隆仓库并按 README 自动完成安装、配置文件创建、校验、模拟运行验证和示例报告生成。若当前工具支持 Skill 目录安装，就把 enterprise-skill/ 安装到对应 skills 目录；若不支持，就把该仓库作为本地工具包使用。请提醒我只在本地配置自己的企业数据接口参数或在线 MCP token，不要提交凭证；除非需要真实接口密钥，否则不要中断询问。
```
...
`enterprise-mcp-server` 是数据接入层。Skill 会自动优先使用 MCP 连接。

### 方式一：使用官方在线 MCP（推荐）

登录 [HandaaS 平台](https://www.handaas.com/)，注册并创建 MCP 对接器，选择 `企业大数据` 在线服务，获取 token。在 HandaaS 平台创建 MCP 对接器时，选择需要的 MCP 在线服务，所选服务共用同一个 token。
...
```bash
export ENTERPRISE_MCP_TOKEN="<your-token>"
...
```powershell
$env:ENTERPRISE_MCP_TOKEN = "<your-token>"
$env:ENTERPRISE_MCP_URL = "https://mcp.handaas.com/enterprise/enterprise_profile"
```

将 `<your-token>` 替换为你在 HandaaS 平台获取的实际 token。token 由 `ENTERPRISE_MCP_TOKEN` 统一携带，URL 中无需重复填写。
...
```bash
export ENTERPRISE_MCP_URL="http://127.0.0.1:8000/mcp"
```

Windows PowerShell：

```powershell
$env:ENTERPRISE_MCP_URL = "http://127.0.0.1:8000/mcp"
```

本地 MCP 由你自己的 `.env` 持有凭证，Skill 侧只需要 `ENTERPRISE_MCP_URL`。
