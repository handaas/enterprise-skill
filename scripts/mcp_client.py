#!/usr/bin/env python3
"""MCP client for the enterprise-report skill.

Connects to the upstream HandaaS MCP server in two ways:

1. **Remote MCP (preferred)** — set ``ENTERPRISE_MCP_URL`` (or the generic
   ``HANDAAS_MCP_URL``) to a streamable-http endpoint. A token may be supplied
   via ``ENTERPRISE_MCP_TOKEN`` / ``HANDAAS_MCP_TOKEN``.
2. **Local MCP (fallback)** — when no URL is configured, the client boots the
   local ``handaas-mcp-server/enterprise-mcp-server/server/mcp_server.py`` over
...
Usage::
    ...
    python mcp_client.py call-tool --tool enterprise_get_enterprise_base_info \
        --arguments-json '{"keyword": "某公司"}'
...
def _require_connection() -> Dict[str, Any]:
    ...
    if not conn:
        raise ConfigError(
            "未配置 MCP 连接。请设置环境变量 ENTERPRISE_MCP_URL（Remote MCP streamable-http），"
