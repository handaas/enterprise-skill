#!/usr/bin/env python3
"""Validate MCP connection config for the enterprise-report skill (no secrets printed)."""
...
def validate(*, allow_placeholders: bool = False) -> Dict[str, Any]:
    ...
    if conn.get("mode") == "remote":
        ...
        warnings.append("未设置 ENTERPRISE_MCP_URL / HANDAAS_MCP_URL；将尝试本地 stdio 连接。")

    local_path = resolve_local_server_path()
    if local_path:
        modes["local_mcp"] = {"ok": True, "server_path": str(local_path)}
    else:
        modes["local_mcp"] = {"ok": False, "error": "未找到本地 MCP server（handaas-mcp-server/enterprise-mcp-server/server/mcp_server.py）。请设置 HANDAAS_MCP_SERVER_ROOT 或 ENTERPRISE_MCP_URL。"}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate enterprise-report MCP connection config.")
